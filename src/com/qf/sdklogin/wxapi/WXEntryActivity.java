...
import com.qf.sdklogin.bean.WxRequestBean;
...
public class WXEntryActivity extends Activity implements IWXAPIEventHandler {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			editor.commit();
			Log.e("sdklogin", "WXEntryActivity handleMessage");
		}
	};
	...
	private void submitThirdLogin(ThirdLoginRequestBean thirdLoginRequestBean) {
		...
		HttpCallbackDecode httpCallbackDecode = new HttpCallbackDecode<LoginResultBean>(mContext,
			...
			public void onDataSuccess(LoginResultBean data) {
				...
				System.exit(0);
				Log.e("sdklogin", "submitThirdLogin22");
			}
	...
	private void getAccessToken(String code) {
		...
		String device_id = getSharedPreferences("sdklogin", Context.MODE_MULTI_PROCESS).getString("device_id", "");
		...
		String deviceinfo = getSharedPreferences("sdklogin", Context.MODE_MULTI_PROCESS).getString("deviceinfo", "");
		...
		wxRequestBean.setCode(code);
		wxRequestBean.setApp_id(appid);
		wxRequestBean.setFrom(from);
		wxRequestBean.setUser_token(usertoken);
		wxRequestBean.setPackagename(packagename);
		wxRequestBean.setClient_id(clientid);
		wxRequestBean.setDevice(deviceBean);
		HttpParamsBuild httpParamsBuild = new HttpParamsBuild(GsonUtil.getGson().toJson(wxRequestBean), mContext);
		HttpCallbackDecode httpCallbackDecode = new HttpCallbackDecode<WxRespBean>(mContext,
				httpParamsBuild.getAuthkey()) {
			@Override
			public void onDataSuccess(WxRespBean resbean) {
			...
			@Override
			public void onFailure(String code, String msg) {
				Log.e("sdklogin", "getAccessToken onFailure msg =" + msg + " ,code =" + code);
				WXEntryActivity.this.finish();
				super.onFailure(code, msg);
			}
		};
		...
		httpCallbackDecode.setShowLoading(false);
		httpCallbackDecode.setLoadMsg("正在登录...");
		RxVolley.post(SdkConstant.GET_ACCESS_TOKEN, httpParamsBuild.getHttpParams(), httpCallbackDecode);
	}
}
